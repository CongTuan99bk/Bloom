...
#define T_SIZE 33554432
#define STR_LENGTH 20
#define SIZE_H 4294967296
...
int check(node_t *head, char str[])
{
    node_t *current =head;
    while(current!=NULL)
    {
               ...
               const char *name, const char *local_port_name)
{
    ...
    // mgr->T=(char *)calloc(T_SIZE,sizeof(char));
    mgr->T=(char *)calloc(SIZE_H,sizeof(char));
    mgr->head=malloc(sizeof(node_t));
    strcpy(mgr->head->data,"header");
    mgr->head->next=NULL;
                       ...
                       const struct ofproto_async_msg *am)
{
   ...
   // FILE *fp;
    ...
    // uint64_t 
    bool check = true;
    unsigned long hash_value[32];       // gia tri hash_value have 40byte
    ...
    dp_packet_use_const(&pkt, am->pin.up.base.packet, am->pin.up.base.packet_len);
    flow_extract(&pkt, &header);
    // fp=fopen("/home/fil02/Desktop/OVS/check.log","a");
    // fp=fopen("/home/fil02/Desktop/OVS/type2.log","a");
    // fprintf(fp, "%d\n",htons(header.tcp_flags));
    // fclose(fp);
    // temp = header.tcp_flags;
    if(htons(header.tcp_flags) == 2) //if captured packet is SYN
    {
        //check = false;
        // fp=fopen("/home/fil02/Desktop/OVS/debug.log","a");
        message = malloc(STR_LENGTH * sizeof(char));
        // for(int i=0; i<10; i++)
        // {
        //     sprintf(message,"%x,%x,%d",header.nw_src,header.tp_src,i);  // 7 byte for all
        //     // fprintf(fp,"%s\n",message);
        //     // SHA256(str,strlen(str),str_out);
        //     // SHA256_CTX ctx;
        //     // SHA256_Init(&ctx);
        //     // SHA256_Update(&ctx,message,7*sizeof(char));
        //     // SHA256_Final(hash,&ctx);
        //     mess = (unsigned char *)message;
        //     SHA256(mess,strlen(message)*sizeof(char),hash);
        //     // for(int x = 0; x < SHA256_DIGEST_LENGTH; x++)
        //         // fprintf(fp,"%02x", hash[x]);
        //     // putchar( '\n' );  

        //     for(int j=0;j<3;j++)
        //     {
        //         hash_value[i]=hash_value[i]<<8;
        //         hash_value[i]+=(int)hash[j];
                
        //     }
        //     for(int j=29;j<32;j++)
        //     {
        //         hash_value2[i]=hash_value2[i]<<8;
        //         hash_value2[i]+=(int)hash[j];
                
        //     }
        //     hash_value[i] += hash_value2[i];
        //     // fprintf(fp, "\n%d, %d\n",hash_value[i], i);
            
        // }
        // free(message);
        // // fclose(fp);
        // if (!(mgr->T[hash_value[0]]=='1' && mgr->T[hash_value[1]]=='1'&&mgr->T[hash_value[2]]=='1'&&mgr->T[hash_value[3]]=='1'&&mgr->T[hash_value[4]]=='1'
        //     &&mgr->T[hash_value[5]]=='1'&&mgr->T[hash_value[6]]=='1'&&mgr->T[hash_value[7]]=='1'&&mgr->T[hash_value[8]]=='1'&&mgr->T[hash_value[9]]=='1'))
        // {
        //         for(int i=0;i<10;i++)
        //             mgr->T[hash_value[i]]='1';
        //         fp=fopen("/home/fil02/Desktop/OVS/debug1.log","a");
        //         fprintf(fp,"notpass\n");
        //         check = false;
        //         fclose(fp);
        //         return;
        ...
        //     for (int i = 0; i < 10; i++)
        //     {
        //         mgr->T[hash_value[i]] = '0';
        //     }
        //     check = true;
        //     fclose(fp);
        // }
           ////////////////////////////////////////////////////
           sprintf(message,"%x,%x",header.nw_src,header.tp_src);  // 7 byte for all
            
            mess = (unsigned char *) message;
            // fprintf(fp,"%s\n",mess);
            SHA256(mess, strlen(message) * sizeof(char), hash);
           
            for(int i=0;i<29;i+=4)  //tinh gia tri 4 byte mot cua ham hash
            {
                for (int j = i; j  < i + 4; j++)
                {
                    hash_value[i] = hash_value[i] << 8;
                    hash_value[i] += (unsigned int)hash[j];
                }
                // fprintf(fp, "\n%ld, %d\n",hash_value[i], i);
            }
        free(message);
        if (!(mgr->T[hash_value[0]]=='1' && mgr->T[hash_value[4]]=='1'&& mgr->T[hash_value[8]]=='1'&&mgr->T[hash_value[12]]=='1'&&mgr->T[hash_value[16]]=='1'
            && mgr->T[hash_value[20]]=='1'&& mgr->T[hash_value[24]]=='1'&& mgr->T[hash_value[28]]=='1'))
        {
                for(int i=0;i<29;i+=4)
                    mgr->T[hash_value[i]]='1';
                // fp=fopen("/home/fil02/Desktop/OVS/debug1.log","a");
    ...
    // fp=fopen("/home/fil02/Desktop/OVS/time.log","a");
    ...
    // {
    //     fp= fopen("/home/fil02/Desktop/OVS/timecheck.log","a+");
    //       fprintf(fp,"%ld, %ld\n",current_t,current_t - last_t);
    //       fclose(fp);
    //       last_t = current_t;
    //       free(mgr->T);
    //     mgr->T=(char *)calloc(SIZE_H,sizeof(char));
