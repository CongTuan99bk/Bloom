    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    *ofprotop = NULL;
    ...
    ofproto->flow_arr=(float *)malloc(DATANUM*sizeof(float));
    for (i=0;i<DATANUM;i++)
    {
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    /**/
    free(ofproto->state);
    free(ofproto->traffic_arr);
    free(ofproto->flow_arr);
...
void abnormal_detect(struct ofproto *p)
{
    ...
    {  
      ...
      struct ofport *port;
      port = ofproto_get_port(p,PORT);   //dinh nghia 1 port
      struct ofputil_port_stats ops = { .port_no = port->pp.port_no };
      ofproto_port_get_stats(port, &ops.stats);
      // now_recorded_n_bytes=(float)ops.stats.rx_bytes;
      // now_recorded_n_flow=(float)get_packet_in_counter();
      // current_flow=(now_recorded_n_flow-p->last_recorded_n_flow)/CYCLE;
      // current_traf=(now_recorded_n_bytes-p->last_recorded_n_bytes)/CYCLE;

      now_recorded_n_bytes=ops.stats.rx_bytes;
      ...
      fp=fopen("/home/clientserver/Desktop/BloomOVS/OVS/data_recording.log","a+");
      ...
      {  
        if(p->data_counter==0)
          fp=fopen("/home/clientserver/Desktop/BloomOVS/OVS/training.log","r");
        ...
        {
          p->writing=1;
          fp=fopen("/home/clientserver/Desktop/BloomOVS/OVS/training.log","a+");
          if (current_flow != 0 && current_traf != 0)  //avoid empty packet
          fprintf(fp, "%f,%f\n",current_flow,current_traf);
          fclose(fp);
          if (current_flow != 0 && current_traf != 0)
          p->data_counter++;
          if(p->data_counter==DATANUM)
          {
            p->writing =0;
            p->data_counter=0;
          }
        ...
        { 
          ...
          while(p->data_counter<DATANUM)
          {
            short ret_val = fscanf(fp,"%f,%f",&p->flow_arr[p->data_counter],&p->traffic_arr[p->data_counter]);
            p->data_counter++;
            if (ret_val == 0)
              printf("no character read \n");
          }
          fclose(fp);              
          p->traffic_mu=calc_mu(p->traffic_arr,p->data_counter);
          p->traffic_sigma=calc_sigma(p->traffic_arr,p->data_counter);
          p->flow_mu=calc_mu(p->flow_arr,p->data_counter);
          p->flow_sigma=calc_sigma(p->flow_arr,p->data_counter);
                  
          for (int i=0;i<p->data_counter;i++)
          { 
            p->Data[i]->traffic_norm=normalize(p->traffic_arr[i],p->traffic_mu,p->traffic_sigma);
            p->Data[i]->n_flow_norm=normalize(p->flow_arr[i],p->flow_mu,p->flow_sigma);
          }
                
          lrd_k(p->Data,p->data_counter);
          fp = fopen("/home/clientserver/Desktop/BloomOVS/OVS/lrd.log","a+");
          for (int i=0;i<p->data_counter;i++)
          {
             fprintf(fp,"%f\n",p->Data[i]->lrd);
          }
          ...
          float temp;
          float max_sec=0;
          LOF_training(p->Data,p->data_counter);
          for(int i=0;i<p->data_counter;i++)
          {
            //temp=1.0*LOF_training(p->Data,p->data_counter);
            //p->Data[i]->lof = temp;
            temp = p->Data[i]->lof;
            if( (p->lof_threshold<temp) && (temp <= 9999) )
              p->lof_threshold=temp;
              else if((p->lof_threshold > temp) && (temp >max_sec))
              {
                  max_sec=temp;
              }
          }
          p->lof_threshold = max_sec;

          //find threshold
          gettimeofday(&end, NULL);
          fp=fopen("/home/clientserver/Desktop/BloomOVS/OVS/time.log","a+");
          fprintf(fp,"%lu\n",(end.tv_sec - start.tv_sec)*1000000+end.tv_usec-start.tv_usec);
          fclose(fp);
          fp=fopen("/home/clientserver/Desktop/BloomOVS/OVS/nor_training.log","a+");
          for(int i=0;i<p->data_counter;i++)
          {
              fprintf(fp,"%f,%f,%f,%f\n",p->Data[i]->n_flow_norm,p->Data[i]->traffic_norm,p->Data[i]->lrd,p->Data[i]->lof);
          }
          fclose(fp);
        }
       // start.tv_sec = 0;
     ...
     else 
      {
          ...
          p->lof_status=LOF(p->state,p->Data,p->data_counter);


          // if(p->lof_status>lof_threshold) 
          //   p->lof_status=1;
          // else p->lof_status=0;
         // fp= fopen("/home/clientserver/Desktop/BloomOVS/OVS/time_detect.log","a+");
          ...
          fp= fopen("/home/clientserver/Desktop/BloomOVS/OVS/debug.log","a+");
      ...
      p->last_recorded_n_bytes=now_recorded_n_bytes;
      p->last_recorded_n_flow=now_recorded_n_flow;
